...
const INITIAL_VISIBLE_COLUMNS = ["title", "tanggal", "update", "comment", "publish", "actions"];

export default function TabelArticle() {
  ...
  const columns = [
    ...
    { uid: "tanggal", name: "Tanggal", sortable: true },
    { uid: "update", name: "Update", sortable: true },
    { uid: "createdby", name: "Created", sortable: true },
    { uid: "comment", name: "Comment", sortable: true },
    { uid: "publish", name: "Publish", sortable: true },
  ...
  const renderCell = React.useCallback((idx, columnKey) => {
    ...
    switch (columnKey) {
      case "category":
        return (
          <span>{cellValue.map((item) => item.category).join(", ")}</span>
        );
        case "tanggal":
        return (
          <span>{formatTanggal(cellValue)}, {formatJam(cellValue)}</span>
        );
        case "update":
        return (
          <span>{formatTanggal(cellValue)}, {formatJam(cellValue)}</span>
      ...
      case "comment":
        return (
          <span>{idx.Comment?.length}</span>
        );
        case "publish":
          return (
            <span>{idx === true ? "Draft" : "Publish"}</span>
          );
      case "actions":
        return (
    ...
    return (
      <div className="flex flex-col gap-4">
        ...
        <div className="flex justify-between items-center">
          <span className="text-small">Total {articles.length} article</span>
  ...
  return (
    ...
    >
      ...
      <TableBody emptyContent={"Tidak ada data article."} items={sortedItems}>
        {(item) => (
