...
export default function Hapus({ params }) {
  ...
  const handleSubmitForm = async (e) => {
    ...
    if (res.ok) {
      toast.success('Berhasil hapus article')
    ...
    <>
      ...
      <Modal isOpen={isOpen} onOpenChange={onOpenChange}>
        ...
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1">Hapus article</ModalHeader>
              <ModalBody>
                <span>Yakit hapus article <span className="text-primary font-bold">{title}</span></span>
              </ModalBody>
              <ModalFooter>
